...
Puppet::Type.type(:openldap_database).provide(:olc) do
  ...
  def self.instances
    databases = slapcat(
      ...
      'cn=config',
      '-H',
      'ldap:///???(|(olcDatabase=monitor)(&(objectClass=olcDatabaseConfig)(|(objectClass=olcBdbConfig)(objectClass=olcHdbConfig)(objectClass=olcMdbConfig)(objectClass=olcMonitorConfig)(objectClass=olcLDAPConfig))))'
    )
    databases.split("\n\n").collect do |paragraph|
      ...
      paragraph.gsub("\n ", "").split("\n").collect do |line|
        case line
        when /^olcDatabase: /
          index, backend = line.match(/^olcDatabase: \{(\d+)\}(bdb|hdb|mdb|monitor|ldap)$/).captures
        when /^olcDbDirectory: /
